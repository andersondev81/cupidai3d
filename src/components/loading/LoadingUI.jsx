...
import CupidLoad from "../../assets/animations/CupidLoad" // Importa o componente de animação
...
const LoadingUI = ({
  ...
  useEffect(() => {
    ...
    const handleError = () => {
      setPhase("error")
      setLoadingText(
        "Ocorreu um erro durante o carregamento. Tentando novamente..."
      )
    }
    ...
    const checkLevaManually = () => {
      ...
      const levaElements = document.querySelectorAll(
        "[data-leva-theme], .leva-c-kWgxhW, .leva-container, .leva-panel"
      )
    ...
    if (isLoaded && !levaReady && !forceReady) {
      securityInterval = setInterval(() => {
        setSecurityTimer(prev => {
          ...
          if (newValue <= 0) {
            console.log(
              "⚠️ Timer de segurança ativado. Habilitando botão de início."
            )
    ...
    return () => {
      ...
      if (isVercel || isFirstVisit) {
        window.removeEventListener(
          "vercel-loading-progress",
          handleVercelProgress
        )
  ...
  useEffect(() => {
    ...
    if (levaElements.length > 0 && !forceReady) {
      console.log(
        "✅ LoadingUI useEffect: Leva detectado manualmente!",
        levaElements
      )
      setForceReady(true)
    }
  ...
  const isButtonVisible =
    phase === "ready" || (isLoaded && (levaReady || forceReady))
      ...
      <div className="w-full max-w-lg mx-auto px-6 text-center">
        ...
        <div className="w-full mb-8 pt-2">
          ...
          <div className="flex justify-center mb-2">
            <div className="relative h-3 w-60 bg-gray-800 rounded-full overflow-hidden mb-2">
              <div
        ...
        <div className="mb-8">
          ...
          {isVercel && isFirstVisit && (
            <p className="text-xs text-pink-400 mt-2">
              Na primeira visita, o carregamento pode ser mais lento. Nas
              próximas visitas será mais rápido!
            </p>
          )}
        </div>
        ...
        {isLoaded && !levaReady && !forceReady && (
          <div className="animate-pulse">
            <p className="text-lg text-pink-400">
              Finalizando inicialização dos controles...
            </p>
